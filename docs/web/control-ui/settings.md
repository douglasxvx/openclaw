...
link. Administrators can use the eye to reveal an entered key or request the saved
literal for that field. A revealed saved value is hidden again when the field,
configuration revision, or Gateway connection changes. The control never resolves
secret references or environment values. Leaving an empty input unchanged
preserves its existing credential.
